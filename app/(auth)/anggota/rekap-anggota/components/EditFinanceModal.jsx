      ...
      <div className="bg-white rounded-xl shadow-lg p-6 w-full max-w-4xl relative space-y-6 overflow-y-auto max-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start bg-white p-2 rounded-lg">
          ...
          <div className="md:col-span-2 flex justify-center">
            <div className="relative w-28 h-36 rounded-xl overflow-hidden shadow-md border-2 border-gray-100">
              <Image
                src={fotoBase64 ? (fotoBase64.startsWith('blob:') ? fotoBase64 : `data:image/jpeg;base64,${fotoBase64}`) : profileImageUrl}
                fill
                alt="Foto User"
                  ...
                  <p>
                    ...
                    <span className="font-medium text-gray-900 uppercase">
                      {dataNpa.tempatLahir || dataNpa.tempat_lahir || dataNpa.user?.tempatLahir || dataNpa.pendaftaran?.tempatLahir || "-"}, {
                        (() => {
                          const tgl = dataNpa.tanggalLahir || dataNpa.tanggal_lahir || dataNpa.user?.tanggalLahir || dataNpa.pendaftaran?.tanggalLahir;
        ...
        <div className="gap-6">
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg border">
            <label className="block text-sm font-medium text-gray-700 mb-3">Tagihan Untuk Bulan</label>
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <label className="block text-xs text-gray-600 mb-1">Dipilih</label>
          ...
          <div className="space-y-2">
            {groupedIuran
              ...
              .map((item, idx) => {
                ...
                return (
                  <div key={idx} className="space-y-2 p-3 rounded-lg bg-white border border-gray-200 hover:border-blue-300 transition-colors">
                    <div className="flex items-center justify-between">
                      <span className="font-semibold text-gray-800 text-sm uppercase tracking-wide">{item.key}</span>
                      <button
                        type="button"
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        ...
                        <input type="text" readOnly value={`Rp. ${oldValue.toLocaleString("id-ID")}`} className="w-full border border-gray-300 px-3 py-2 rounded text-center bg-gray-50 text-gray-700 font-medium" />
                      </div>
                      <div>
                        <label className="block text-xs text-gray-500 mb-1">Tambahan Cabang</label>
                        <input
                          type="text"
                          placeholder="Rp. 0"
                          value={inputValue === 0 ? "" : `Rp. ${inputValue.toLocaleString("id-ID")}`}
                          onChange={(e) => {
                            const angka = parseInt(e.target.value.replace(/[^\d]/g, "")) || 0;
                            setNominalBaruList((prev) => ({ ...prev, [item.key]: angka }));
                          }}
                      ...
                      <div>
                        ...
                        <input type="text" readOnly value={`Rp. ${totalValue.toLocaleString("id-ID")}`} className="w-full border border-green-300 px-3 py-2 rounded text-center bg-green-50 text-green-700 font-medium" />
                      </div>
                    </div>
                  </div>
                );
            ...
            {sumbanganList.length > 0 && (
              <div className="mt-4">
                <h4 className="font-semibold text-purple-700 mb-2">Keuangan:</h4>
                {sumbanganList.map((sumbangan, index) => {
                  const jumlahValue = parseInt(sumbangan.jumlah || 0);
                  const isDeleted = jumlahValue === 0;
                  return (
                    <div key={index} className={`space-y-1 px-3 py-2 rounded-md border-l-4 mb-2 ${isDeleted ? "bg-gray-100 border-gray-400 opacity-60" : "bg-purple-50 border-purple-400"}`}>
                      <div className="flex items-center justify-between">
                        <span className={`font-medium ${isDeleted ? "text-gray-500" : "text-purple-800"}`}>
                          {sumbangan.jenis} {isDeleted && <span className="text-red-500 ml-2 text-sm">(Dihapus)</span>}
                        </span>
                        <button
                          type="button"
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-2">
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Nilai Sumbangan</label>
                          <input type="text" readOnly value={`Rp. ${jumlahValue.toLocaleString("id-ID")}`} className={`w-full border px-2 py-1 rounded text-center font-medium ${isDeleted ? "bg-gray-200 text-gray-500" : "bg-purple-100 text-purple-700"}`} />
                        </div>
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Tambahan Cabang</label>
                          <input type="text" readOnly value={isDeleted ? "Dihapus" : "Tidak bisa diubah"} className="w-full border px-2 py-1 rounded text-center bg-gray-200 text-gray-500" />
                        </div>
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Total</label>
                          <input type="text" readOnly value={`Rp. ${jumlahValue.toLocaleString("id-ID")}`} className={`w-full border px-2 py-1 rounded text-center font-medium ${isDeleted ? "bg-gray-200 text-gray-500" : "bg-purple-100 text-purple-700"}`} />
            ...
            {addedCategories.map((item, idx) => {
              ...
              return (
                <div key={`added-${idx}`} className="space-y-2 p-3 rounded-lg bg-yellow-50 border-l-4 border-yellow-400 hover:border-yellow-500 transition-colors">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setManualInputs((prev) => { const n = { ...prev }; delete n[item.key]; return n; });
                        setNewValues((prev) => { const n = { ...prev }; delete n[item.key]; return n; });
                      }}
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="block text-xs text-yellow-600 mb-1">Default</label>
                      <input type="text" readOnly value={`Rp. ${oldValue.toLocaleString("id-ID")}`} className="w-full border border-yellow-300 px-3 py-2 rounded text-center bg-yellow-100 text-yellow-700 font-medium" />
                    </div>
                    <div>
                      <label className="block text-xs text-yellow-600 mb-1">Tambahan</label>
                      <input
                        type="text"
                        placeholder="Rp. 0"
                        value={inputValue === 0 ? "" : `Rp. ${inputValue.toLocaleString("id-ID")}`}
                        onChange={(e) => {
                          const angka = parseInt(e.target.value.replace(/[^\d]/g, "")) || 0;
                          setManualInputs((prev) => ({ ...prev, [item.key]: angka }));
                        }}
                    ...
                    <div>
                      <label className="block text-xs text-yellow-600 mb-1">Total</label>
                      <input type="text" readOnly value={`Rp. ${totalValue.toLocaleString("id-ID")}`} className="w-full border border-yellow-300 px-3 py-2 rounded text-center bg-yellow-100 text-yellow-700 font-medium" />
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg border border-gray-300 p-4 mt-6">
              <h3 className="text-lg font-semibold text-purple-800 mb-3 flex items-center">
                <span className="bg-purple-600 text-white p-2 rounded-full mr-3"><FiPlus size={18} /></span>
                Tambah Keuangan
              </h3>
              <button
              ...
              >
                <span className="bg-blue-100 text-blue-600 p-2 rounded-full mr-3"><FiPlus size={16} /></span>
              ...
              {showDropdown && (
                <div className="space-y-4 bg-white p-4 rounded-lg border border-blue-200">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Pilih Kategori Tambahan</label>
                    <select
                    ...
                    >
                      ...
                      <option value="daspen">Daspen {notifDaspen === true ? " (✓ Sinkron)" : " (× Tidak Sinkron)"}</option>
                  ...
                  {selectedKategori && (
                    <div className="flex justify-end space-x-3 pt-2">
                      <button onClick={() => setShowDropdown(false)} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Batal</button>
                      <button onClick={handleSave} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-medium transition-colors flex items-center">
        ...
        <div className="flex justify-end gap-4 pt-4">
          <button className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded" onClick={closePopup}>Batal</button>
          <button
            type="button"
