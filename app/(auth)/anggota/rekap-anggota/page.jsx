...
function RekapAnggota() {
  ...
  const groupedIuran = useMemo(() => {
    ...
    const filteredExtra = extra.filter(
      (e) => !baseKeys.includes(e.key) && !existingMap.has(e.key),
    );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const actualStd = Array.isArray(responseStd)
          ? responseStd
          : responseStd?.data || [];
  ...
  const stats = useMemo(() => {
    ...
    data.forEach((item) => {
      ...
      initial.sumbangan += getTotalSumbangan(item);
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        rekapDropdownRef.current &&
        !rekapDropdownRef.current.contains(event.target)
      ) {
  ...
  const fetchInitialData = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(filesResponse)) {
          filesResponse.forEach((f) => {
            if (f.nip) fileMap[f.nip] = f.sumbangan;
          });
      ...
      const regularData = syncedData.filter(
  ...
  const handleCabangSearch = (query) => {
    ...
    const filtered = originalCabangList.filter((c) =>
  ...
  const handleUnitKerjaFocus = () => {
    if (selectedCabang) setShowUnitKerjaDropdown(true);
  };

  const handleUnitKerjaClick = () => {
    if (!selectedCabang) return;
    const filtered = unitKerjaList.filter(
      (u) => u.cabang?.toLowerCase() === selectedCabang.toLowerCase(),
    );
    setFilteredUnitKerja(filtered);
    setShowUnitKerjaDropdown(true);
  };
  ...
  const performSearch = (query) => {
    const filtered = originalRekapData.filter((item) => {
      ...
      const matchName =
        !query || item.namaAnggota?.toLowerCase().includes(query.toLowerCase());
  ...
  const handleMemberClick = useCallback(async (member) => {
    ...
    try {
      ...
      const freshMember = {
        ...member,
        pgri:
          dataIuran.defaultPgri && parseInt(dataIuran.defaultPgri) > 0
            ? parseInt(dataIuran.defaultPgri)
            : dataIuran.pgri && parseInt(dataIuran.pgri) > 0
              ? parseInt(dataIuran.pgri) - (parseInt(dataIuran.manualPgri) || 0)
              : member.pgri > 0
                ? parseInt(member.pgri)
                : parseInt(profile.pgri) || 8000,
        sanduka:
          dataIuran.defaultSanduka && parseInt(dataIuran.defaultSanduka) > 0
        ...
        daspen:
          dataIuran.defaultDaspen && parseInt(dataIuran.defaultDaspen) > 0
        ...
        derap:
          dataIuran.defaultDerap && parseInt(dataIuran.defaultDerap) > 0
        ...
        kalender:
          dataIuran.defaultKalender && parseInt(dataIuran.defaultKalender) > 0
  ...
  const handleDeleteSumbangan = (jenis) => {
    setSumbanganList((prev) =>
      prev.map((s) => (s.jenis === jenis ? { ...s, jumlah: 0 } : s)),
    );
  };
  ...
  const handleSave = async () => {
            ...
            initialValue = Object.entries(data).reduce((acc, [k, v]) => {
              if (
                [
                  ...
                  "jumlah",
                  "tarif",
                ].some((key) => k.toLowerCase().includes(key)) &&
    ...
    // =========================
      ...
      let pgriItem = currentStdList.find(
        (item) => item.iuran?.trim().toUpperCase() === "IURAN PGRI",
      );
        ...
        ...prev,
        {
          ...
          label:
            selectedKategori === "lainlain"
              ? uniqueKey
              : labelMap[selectedKategori] || uniqueKey,
  ...
  const handleUpdateClick = async () => {
    ...
    try {
      ...
      const success = await handleUpdateIuran(
        selectedMember.idByNominal,
        payload,
      );
  ...
  const confirmBackupTarget = async () => {
    ...
    try {
      ...
      setNotification({
        type: "success",
        message:
          typeof res === "string" ? res : "Backup Target berhasil diproses!",
      });
      setIsBackupModalVisible(false);
    } catch (err) {
      console.error("Backup Target error:", err);
      const errorMsg =
        err.response?.data || err.message || "Gagal memproses Backup Target.";
      setNotification({
        type: "error",
        message:
          typeof errorMsg === "string"
            ? errorMsg
            : "Gagal memproses Backup Target.",
      });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-50 to-white p-0">
      ...
      <div className="flex">
        ...
        <div
          className={`flex-1 transition-all duration-300 mt-12 px-4 md:px-8 ${isSidebarOpen && !isMobile ? "ml-64" : "ml-0"}`}
        >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-medium">
                    Periode penagihan yang sedang diproses
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="overflow-x-auto bg-white rounded-b-lg shadow-xl border border-teal-100">
              <table className="w-full text-left border-collapse md:min-w-[1200px]">
                <thead className="hidden md:table-header-group bg-teal-600 text-white text-sm uppercase sticky top-0 z-10">
                  <tr>
                    <th className="p-3 border-b border-teal-500 text-center w-12">
                      No
                    </th>
                    <th className="p-3 border-b border-teal-500">Cabang</th>
                    <th className="p-3 border-b border-teal-500">Unit Kerja</th>
                    <th className="p-3 border-b border-teal-500">
                      Nama Anggota
                    </th>
                    <th className="p-3 border-b border-teal-500 text-right">
                      PGRI
                    </th>
                    <th className="p-3 border-b border-teal-500 text-right">
                      Sanduka
                    ...
                    <th className="p-3 border-b border-teal-500 text-center">
                      Aksi
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    <tr>
                      <td
                        colSpan={12}
                        className="p-10 text-center text-teal-600 font-medium"
                      >
                        Loading data...
                      </td>
                    </tr>
