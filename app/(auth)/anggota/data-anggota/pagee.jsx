...
const DataAnggota = () => {
  ...
  const handleCabangChange = (value) => {
    ...
    setCurrentPage(0);
    fetchDataAnggota(0, pageSize, selectedKecamatan, "", nama, status);
  };

  const handleUnitKerjaChange = (value) => {
    setSelectedUnitKerja(value);
    setCurrentPage(0);
    fetchDataAnggota(0, pageSize, selectedCabang, value, nama, status);
  };
...
const DataTable = ({
  ...
  return (
    <div>
      <table className="min-w-full bg-white border-0 rounded-xl shadow-lg overflow-hidden">
        <thead>
          <tr className="bg-gradient-to-r from-teal-700 to-teal-600">
            <th className="p-3 md:p-4 text-white text-left text-xs font-semibold uppercase tracking-wider">
              No
            </th>
            <th className="p-3 md:p-4 text-white text-center text-xs font-semibold uppercase tracking-wider">
              Foto
            </th>
            <th className="p-3 md:p-4 text-white text-left text-xs font-semibold uppercase tracking-wider">
              Nama & Identitas
            </th>
            <th className="p-3 md:p-4 text-white text-left text-xs font-semibold uppercase tracking-wider md:table-cell hidden">
              Tanggal Lahir
            </th>
            <th className="p-3 md:p-4 text-white text-left text-xs font-semibold uppercase tracking-wider md:table-cell hidden">
              Unit Kerja
            </th>
            <th className="p-3 md:p-4 text-white text-center text-xs font-semibold uppercase tracking-wider md:table-cell hidden">
              Keterangan
            </th>
            <th className="p-3 md:p-4 text-white text-center text-xs font-semibold uppercase tracking-wider md:table-cell hidden">
              Lokasi
            ...
            (anggotaData || []).map((item, index) => {
              ...
              return (
                <React.Fragment key={item.id}>
                  <tr className={`text-sm transition-colors duration-150 ${index % 2 === 0 ? "bg-white" : "bg-gray-50/60"} hover:bg-teal-50/40`}>
                    ...
                    {isMobile ? (
                      <td className="py-3 px-3 align-middle">
                        <div className="w-14 h-14 flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity">
                          <Image
                            onClick={() => setZoomedImage(fotoBase64[index] ? `data:image/jpeg;base64,${fotoBase64[index]}` : profileImageUrl)}
                            src={fotoBase64[index] ? `data:image/jpeg;base64,${fotoBase64[index]}` : profileImageUrl}
                            width={56} height={56}
                            alt="Anggota Foto"
                            className="object-cover rounded-full border-2 border-teal-200 shadow-sm"
                            unoptimized={true}
                          />
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-3 align-middle text-center">
                        <div className="w-14 h-14 mx-auto overflow-hidden flex items-center justify-center cursor-pointer hover:opacity-80 transition-opacity">
                          <Image
                            onClick={() => setZoomedImage(fotoBase64[index] ? `data:image/jpeg;base64,${fotoBase64[index]}` : profileImageUrl)}
                            src={fotoBase64[index] ? `data:image/jpeg;base64,${fotoBase64[index]}` : profileImageUrl}
                            width={56} height={56}
                    ...
                    <td className="py-3 px-4 align-middle">
                      <div className="font-semibold text-gray-800 text-sm">{item.namaLengkap}</div>
                      <div className="text-xs text-gray-500 mt-0.5">{item.npaPgri}</div>
                      <div className="text-xs text-gray-500">{item.jabatan}</div>
                      <span className={`mt-1 text-xs px-2 py-0.5 rounded-full font-medium inline-block ${item.nip && item.nip !== "0" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-600"}`}>
                        {item.nip && item.nip !== "0" ? item.nip : "Tidak Terdaftar Daspen"}
                      </span>
                    </td>
                    {!isMobile && (
                      <>
                        <td className="py-3 px-4 align-middle text-sm text-gray-600">
                          <div className="font-medium text-gray-700">{item.tempatLahir}</div>
                          <div>{formatDate(item.tanggalLahir)}</div>
                          <div className="text-xs text-gray-500">{calculateAge(item.tanggalLahir)}</div>
                          <div className="text-xs text-orange-600 font-medium mt-0.5">
                            Pensiun: {item.prediksiPensiun ? formatRetirementDate(item.prediksiPensiun) : "-"}
                          </div>
                        </td>
                        <td className="py-3 px-4 align-middle text-sm text-gray-600">
                          <div className="font-medium text-gray-700">{item.cabang}</div>
                          <div>{item.unitKerja}</div>
                          ...
                          <div className="text-xs text-gray-500">{item.pangkatGolongan}</div>
                        </td>
                        <td className="py-3 px-4 align-middle text-center">
                          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${item.statusKeanggotaan === "Aktif" ? "bg-green-100 text-green-700" : item.statusKeanggotaan === "Pensiun" ? "bg-yellow-100 text-yellow-700" : item.statusKeanggotaan === "Meninggal" ? "bg-gray-100 text-gray-600" : "bg-red-100 text-red-600"}`}>
                            {item.statusKeanggotaan}
                          </span>
                          <div className="text-xs text-gray-500 mt-1">{item.nomorRekening || "-"}</div>
                          <div className={`text-xs mt-1 font-medium px-2 py-0.5 rounded-full inline-block ${filesByNip.find(f => String(f?.nip) === String(item.nip))?.verifikasi === true ? "bg-teal-100 text-teal-700" : "bg-red-100 text-red-600"}`}>
                            {filesByNip.find(f => String(f?.nip) === String(item.nip))?.verifikasi === true ? "✓ Daspen" : "✗ Daspen"}
                          </div>
                        </td>
                        <td className="py-3 px-4 align-middle text-center">
                          <button
                          ...
                          >
                            📍 {item.latitude ? `${item.latitude.toFixed(4)}, ${item.longitude.toFixed(4)}` : "Tidak tersedia"}
                          </button>
                        </td>
                        <td className="py-3 px-3 align-middle">
                  ...
                  {expandedRow === index && (
                    <tr>
                      <td colSpan="9" className="px-4 py-4 bg-teal-50/30 border-t border-teal-100">
                        <div className="flex flex-col items-center space-y-4">
                          <div className="grid grid-cols-2 gap-4 w-full max-w-lg">
                            <div className="bg-white rounded-xl p-3 shadow-sm border border-gray-100">
                              <h3 className="font-semibold text-teal-700 text-xs uppercase tracking-wider mb-2">Tanggal Lahir</h3>
                              <div className="text-sm text-gray-700">{item.tempatLahir}</div>
                              <div className="text-sm text-gray-700">{formatDate(item.tanggalLahir)}</div>
                              <div className="text-xs text-gray-500">{calculateAge(item.tanggalLahir)}</div>
                            ...
                            <div className="bg-white rounded-xl p-3 shadow-sm border border-gray-100">
                              <h3 className="font-semibold text-teal-700 text-xs uppercase tracking-wider mb-2">Unit Kerja</h3>
                              <div className="text-sm text-gray-700">{item.cabang}</div>
                              <div className="text-sm text-gray-700">{item.unitKerja}</div>
                              <div className="text-xs text-gray-500 mt-1">
                                Anggota: {item.tahunDiangkat ? (() => { const d = new Date(item.tahunDiangkat); return `${String(d.getDate()).padStart(2,"0")}-${String(d.getMonth()+1).padStart(2,"0")}-${d.getFullYear()}`; })() : "-"}
                              </div>
                              <div className="text-xs text-gray-500">{item.pangkatGolongan}</div>
                            </div>
                            <div className="bg-white rounded-xl p-3 shadow-sm border border-gray-100">
                              <h3 className="font-semibold text-teal-700 text-xs uppercase tracking-wider mb-2">Status</h3>
                              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${item.statusKeanggotaan === "Aktif" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}>
                                {item.statusKeanggotaan}
                              </span>
                            </div>
                          </div>
