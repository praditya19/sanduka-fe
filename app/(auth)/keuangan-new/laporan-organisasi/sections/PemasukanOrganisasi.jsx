...
const PemasukanOrganisasiSection = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await GlobalApi.getPemasukanUmum();
      setData(Array.isArray(res) ? res : []);
    } catch (error) {
      console.error("Error fetching pemasukan organisasi:", error);
  ...
  const filteredData = data.filter((item) => {
    ...
    const cabang =
      typeof item.cabang === "object"
        ...
        : item.cabang || item.namaCabang || item.nama_cabang || "";
    return (
      (item.keterangan || "").toLowerCase().includes(q) ||
      cabang.toLowerCase().includes(q) ||
    ...
    const excelData = filteredData.map((item, index) => ({
      ...
      Cabang:
        typeof item.cabang === "object"
          ? item.cabang?.kecamatan || item.cabang?.cabang || item.cabang?.namaCabang
          : item.cabang || item.namaCabang || item.nama_cabang || "-",
      Nominal: item.nominal || item.debet || item.debit || 0,
      "No. Bukti": item.nomorBukti || "-",
    }));

    const ws = XLSX.utils.json_to_sheet(excelData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Pemasukan Organisasi");
    XLSX.writeFile(wb, `Pemasukan_Organisasi_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
      ...
      <div className="bg-emerald-600 p-6 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-xl font-black">Pemasukan Organisasi</h2>
              <p className="text-emerald-100 text-xs font-medium uppercase tracking-wider">
                Seluruh transaksi pemasukan kas organisasi
              </p>
            </div>
          </div>
      ...
      <div className="p-6 space-y-6 flex-1 overflow-y-auto">
        <div className="bg-slate-50 p-4 rounded-[24px] border border-slate-100 flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[200px] relative">
            ...
            <input
              type="text"
              placeholder="Ketik keterangan, cabang, atau no. bukti..."
              value={searchQuery}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-900 text-white text-[10px] uppercase font-black tracking-widest">
                  ...
                  <th className="px-6 py-4 border-r border-white/10">Cabang</th>
                  <th className="px-6 py-4 text-right border-r border-white/10">Nominal</th>
                  <th className="px-6 py-4 text-center">No. Bukti</th>
                </tr>
              </thead>
                    ...
                    <tr key={i} className="hover:bg-emerald-50/30 transition-colors group">
                      ...
                      <td className="px-6 py-4 border-r border-slate-50">
                        <span className="font-bold text-slate-700 uppercase text-xs">
                          {typeof row.cabang === "object"
                            ? row.cabang?.kecamatan || row.cabang?.cabang || row.cabang?.namaCabang
                            : row.cabang || row.namaCabang || row.nama_cabang || "-"}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right font-black text-emerald-600 border-r border-slate-50 bg-emerald-50/20">
                        {formatCurrency(row.nominal || row.debet || row.debit)}
                      </td>
                  ...
                  <tr>
                    <td colSpan="6" className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-slate-400 font-bold">Data pemasukan tidak ditemukan</p>
                      </div>
                    </td>
                  </tr>
                )}
                ...
                <tfoot className="bg-slate-50 border-t-2 border-slate-100">
                  <tr className="font-black text-slate-800 text-xs">
                    ...
                    <td className="px-6 py-4 text-right text-emerald-700 bg-emerald-100/30">
                      {formatCurrency(filteredData.reduce((acc, curr) => acc + (Number(curr.nominal || curr.debet || curr.debit) || 0), 0))}
                    </td>
                    <td></td>
                  </tr>
                </tfoot>
...
export default PemasukanOrganisasiSection;
