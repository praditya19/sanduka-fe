...
const TahunanOrganisasi = () => {
  const [loading, setLoading] = useState(false);
  const [pemasukan, setPemasukan] = useState([]);
  const [pengeluaran, setPengeluaran] = useState([]);
  const [activeSubTab, setActiveSubTab] = useState("pemasukan");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      for (let bulan = 1; bulan <= 12; bulan++) {
        try {
          const res = await GlobalApi.getTableKasOrganisasi(bulan, selectedYear);
          const data = Array.isArray(res) ? res : res?.data || [];

          allData.push(...data);
        } catch (err) {
          console.error(`❌ Error bulan ${bulan}:`, err);
        }
      }
      ...
      allData
        ...
        .forEach((item) => {
          const bulan = item.tanggalTransaksi?.[1];

          if (!pemasukanMap[bulan]) {
            pemasukanMap[bulan] = 0;
          }

          pemasukanMap[bulan] += Number(item.debet || 0);
        });

      const pemasukanFormatted = ALL_MONTHS.map((bulan) => ({
        bulan,
        nominal: pemasukanMap[bulan] || 0,
      }));
      ...
      allData
        ...
        .forEach((item) => {
          ...
          if (!pengeluaranMap[bulan]) {
            pengeluaranMap[bulan] = 0;
          }

          pengeluaranMap[bulan] += Number(item.kredit || 0);
        });

      const pengeluaranFormatted = ALL_MONTHS.map((bulan) => ({
        bulan,
        totalPengeluaran: pengeluaranMap[bulan] || 0,
      }));

      setPemasukan(pemasukanFormatted);
      setPengeluaran(pengeluaranFormatted);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-6 space-y-6 flex-1 overflow-y-auto">
        ...
        {/* Total Summary */}
        <div
          className={`border p-6 rounded-[32px] flex items-center justify-between ${activeSubTab === "pemasukan" ? "bg-emerald-50 border-emerald-100" : "bg-rose-50 border-rose-100"}`}
        >
          <div className="flex items-center space-x-4">
            <div
              className={`w-12 h-12 text-white rounded-2xl flex items-center justify-center text-2xl shadow-lg ${activeSubTab === "pemasukan" ? "bg-emerald-500" : "bg-rose-500"}`}
            >
            ...
            <div>
              ...
              <p
                className={`text-[10px] font-bold ${activeSubTab === "pemasukan" ? "text-emerald-600" : "text-rose-600"}`}
              >
          ...
          >
            {formatCurrency(
              currentData.reduce((acc, curr) => {
                if (activeSubTab === "pemasukan") {
                  return acc + (curr.nominal || 0);
                } else {
                  return acc + (curr.totalPengeluaran || 0);
                }
              }, 0),
            )}
          </span>
...
export default TahunanOrganisasi;
